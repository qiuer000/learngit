...
public class CxGsActivity extends Activity{
	...
	private String HTTP = "http://2001.offerxiaomi.sinaapp.com/easy/testgs.php";// 请求连接的网址
	private HttpClient client;
	public String input_gongsi = "";// 默认公司输入为空
	public String input_uni_city = "";// 默认学校或者城市输入为空
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.cx_gs_main);
...
//		timeseek = bundle.getString("time");// 获得时间选项值
		readNet(HTTP, input_gongsi, input_uni_city,inputPage);// 执行查询命令

		// 返回按钮，结束当前CxActivity,返回宣讲会界面
		Button_back.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				CxGsActivity.this.finish();
			}
		});
		...
		Button_previousPage.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (nowPage != 1) {
					INFO="";//先将原内容置空
					readNet(HTTP, input_gongsi, input_uni_city,
							(nowPage - 1));// 如果当前页数不是第1页则执行
				} else {
					Toast.makeText(CxGsActivity.this, "提示:已经是第1页了!",
		...
		Button_nextPage.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (nowPage != sumPage) {
					...
					Toast.makeText(CxGsActivity.this, "提示:已经是最后一页了!",
							Toast.LENGTH_LONG).show();
				}
				EditText_inputPage.setText(null);// 清空页码输入框
			}
		...
		Button_jump.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (EditTextinputPage <= sumPage && EditTextinputPage >= 1) {
					...
					Toast.makeText(CxGsActivity.this, "提示:输入页码有误!",
							Toast.LENGTH_LONG).show();
				}
	...
	public void readNet(String url,  String company, String uni , final int inputPage) {
		//判断网络连接状态再执行后续程序
				...
				if (network_state.getActiveNetworkInfo()==null) {
					Toast.makeText(CxGsActivity.this, "当前网络不可用，请检查网络设置", Toast.LENGTH_SHORT).show();//检查网络连接，如果无网络可用，就不进行后续操作等
		...
		new AsyncTask<String, Void, String>() {
			...
			protected String doInBackground(String... arg0) {
				...
				try {
					...
					parameters.add(new BasicNameValuePair("company", input_gongsi));//公司#科技input_gongsi
					parameters.add(new BasicNameValuePair("uid", input_uni_city));//input_uni_city
			...
			protected void onPostExecute(String result) {
				try {
					...
					if (root.getString("flag").equals("2")) {
