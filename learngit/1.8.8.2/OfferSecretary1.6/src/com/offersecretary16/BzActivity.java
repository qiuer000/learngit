...
public class BzActivity extends Activity{
	...
	private TextView bz_print;// 内容显示框
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		bz_print=(TextView) findViewById(R.id.bz_print);
		bz_print.setMovementMethod(ScrollingMovementMethod.getInstance());// 设置滚动条
		...
		// 返回主页按钮响应
				Button_back.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View arg0) {
						BzActivity.this.finish();// 结束当前BzActivity
					}
				});
	}
	...
	public void readNet(String url) {
		//判断网络连接状态再执行后续程序
				...
				if (network_state.getActiveNetworkInfo()==null) {
					Toast.makeText(BzActivity.this, "当前网络不可用，请检查网络设置", Toast.LENGTH_SHORT).show();//检查网络连接，如果无网络可用，就不进行后续操作等
					bz_print.setText("当前网络不可用，请检查网络设置");// 显示网络问题到界面的显示框
					bz_print.setTextColor(Color.RED);//设置文字颜色为红色
				}else{
		new AsyncTask<String, Void, String>() {
			...
			protected String doInBackground(String... arg0) {
				...
				try {
					...
					parameters.add(new BasicNameValuePair("uid", "帮助"));
			...
			protected void onPostExecute(String result) {
				try {
					...
					bz_print.setText(INFO);// 显示内容到界面的显示框
