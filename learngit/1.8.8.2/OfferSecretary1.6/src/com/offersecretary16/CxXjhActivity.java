...
public class CxXjhActivity extends Activity {
	...
	private String HTTP = "http://2001.offerxiaomi.sinaapp.com/easy/testcai4.php";// 请求连接的网址
	private HttpClient client;
	public String cityseek = "";// 默认城市选项为空
	public String uniseek = "";// 默认大学选项为空
	...
	public String timeseek = "";// 默认时间选项为空
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.cx_xjh_main);
		...
		text.setMovementMethod(LinkMovementMethod.getInstance());//设置文本超链接响应                                                                                      //12.8添加
		text.setAutoLinkMask(Linkify.WEB_URLS); //自动识别超链接							//12.8添加
		text.setFocusable(true);; //							//12.8添加
		text.setEnabled(true); //						//12.8添加
		cityseek = bundle.getString("city");// 获得城市选项值
		uniseek = bundle.getString("uni");// 获得时大学选项值
		...
		timeseek = bundle.getString("time");// 获得时间选项值
		readNet(HTTP, cityseek, uniseek, dateseek, timeseek, inputPage);// 执行查询命令			

		// 返回按钮，结束当前CxActivity,返回宣讲会界面
		Button_back.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				CxXjhActivity.this.finish();
			}
		});
		...
		Button_previousPage.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (nowPage != 1) {
					INFO="";//先将原内容置空
						readNet(HTTP, cityseek, uniseek, dateseek, timeseek,
								(nowPage - 1));// 如果当前页数不是第1页则执行
				} else {
					Toast.makeText(CxXjhActivity.this, "提示:已经是第1页了!",
		...
		Button_nextPage.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (nowPage != sumPage) {
					...
					Toast.makeText(CxXjhActivity.this, "提示:已经是最后一页了!",
							Toast.LENGTH_LONG).show();
				}
				EditText_inputPage.setText(null);// 清空页码输入框
			}
		...
		Button_jump.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (EditTextinputPage <= sumPage && EditTextinputPage >= 1) {
					...
					Toast.makeText(CxXjhActivity.this, "提示:输入页码有误!",
							Toast.LENGTH_LONG).show();
				}
	...
	public void readNet(String url, String city, String uni, String date,
			String time, final int inputPage) {
		//判断网络连接状态再执行后续程序
		...
		if (network_state.getActiveNetworkInfo()==null) {
			Toast.makeText(CxXjhActivity.this, "当前网络不可用，请检查网络设置", Toast.LENGTH_SHORT).show();//检查网络连接，如果无网络可用，就不进行后续操作等
		...
		new AsyncTask<String, Void, String>() {
			...
			protected String doInBackground(String... arg0) {
				...
				try {
					...
					parameters.add(new BasicNameValuePair("uni", uniseek));
					if(dateseek.equals("-"+"-")){
						dateseek="";//如果日期没有输入，则置日期为空
					}
					parameters.add(new BasicNameValuePair("date", dateseek));// 日期默认为空
					parameters.add(new BasicNameValuePair("time", timeseek));
			...
			protected void onPostExecute(String result) {
				try {
					...
					if (root.getString("flag").equals("1")) {
						text_yindao.setText("宣讲会信息：\n总共" + sumPage + "页"
...
//									+ ("链接：" +Html.fromHtml("<a href=\""+lan.getString("description")+"\">请点击查询</a> ")+ "\n")			//	12.8添加		再注
									+ "--------------------------" + "\n"						
									;
						}
					} else {
		...
		}.execute(url, city, uni, date, time);
		}
	}
}
